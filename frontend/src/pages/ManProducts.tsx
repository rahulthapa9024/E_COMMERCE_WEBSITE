...
export default function MansPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const prodRes = await axiosClient.get("/product/getAllMenProducts");
      setProducts(prodRes.data.products || prodRes.data || []);
      
      // Fetch cart and favorites if user is logged in
      if (emailId) {
        try {
          ...
          console.error("Failed to fetch cart/favorites", err);
          setCart([]);
          setFavourites([]);
        }
      } else {
  ...
  const toggleFav = async (title: string) => {
    ...
    try {
      ...
      fetchData(); // Refresh favorites data
    } catch (err) {
      ...
      <header className="relative pt-24 pb-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto text-center">
          ...
          <p className="text-base sm:text-lg md:text-xl text-zinc-400 max-w-2xl mx-auto mb-8 tracking-wide px-4">
            Engineered for the modern silhouette. Superior craft and performance.
          </p>
        </div>
      </header>

      {/* FILTER BAR (Exact Home Copy) */}
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between py-4 gap-4">
            ...
            <div className="relative w-full lg:w-auto lg:flex-1 max-w-2xl">
              ...
              <input
                ...
                placeholder="Search men's products, colors, sizes..."
                value={searchQuery}
              ...
              {filteredProducts.map(p => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Image */}
                    <div 
                      className="relative aspect-[3/4] overflow-hidden bg-gradient-to-br from-zinc-900 to-black"
            ...
            {filteredProducts.length === 0 && !isLoading && (
              <div className="text-center py-12 xs:py-16 md:py-24">
                <div className="max-w-md mx-auto px-4">
                  ...
                  <h3 className="text-lg xs:text-xl font-semibold mb-2">No men's products found</h3>
                  <p className="text-zinc-400 text-sm xs:text-base mb-6">
                    {searchQuery
                      ? `No men's products match "${searchQuery}". Try different keywords.`
                      : "No men's products available with the current filters."}
                  </p>
                  <button
                    onClick={() => {
                      setSearchQuery("");
